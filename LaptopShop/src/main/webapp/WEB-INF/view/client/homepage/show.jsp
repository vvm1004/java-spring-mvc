                  ...
                  <c:forEach var="product" items="${products}">
                    ...
                    <c:if test="${not empty product.image}">
                      <c:choose>
                        ...
                        <c:otherwise>
                          <c:set var="imgSrc" value="/images/product/${product.image}" />
                        </c:otherwise>
                      </c:choose>
                    </c:if>
                      ...
                      <div class="rounded position-relative fruite-item product-card w-100">
                        <div class="fruite-img">
                          <a href="/product/${product.id}">
                            <img src="${imgSrc}" class="img-fluid w-100 rounded-top product-img-fixed" alt="${product.name}">
                          </a>
                        </div>
                        <div class="text-white bg-secondary px-3 py-1 rounded position-absolute"
                          style="top: 10px; left: 10px;">Laptop</div>
                        <div class="p-4 border border-secondary border-top-0 rounded-bottom product-card-body">
                          <h4 class="product-card-title"><a href="/product/${product.id}">${product.name}</a></h4>
